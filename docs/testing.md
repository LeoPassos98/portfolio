...
| Último resultado consolidado | **307 testes aprovados** na alteração de e-mail de login da conta de acesso no backend                    |
...
Salvo a exceção indicada no smoke e2e, os arquivos `*.spec.ts` deste catálogo foram aprovados como parte da suíte de **307 testes** executada na alteração de e-mail de login da conta de acesso no backend. Os resultados são cumulativos: não representam a quantidade criada por arquivo ou família.
...
Exercita criação, edição cadastral, situações do cadastro e da conta, perfil e e-mail de login da conta, criação explícita de conta e consultas administrativas de Funcionários pelas rotas reais.
...
| Privacidade e OpenAPI | Não expõe hash, senha, confirmação, troca obrigatória, sessão, CSRF, OS ou histórico; documenta criação, criação de conta, edição cadastral, situações, perfil, e-mail de login, conflitos e `conta` anulável.                                                                                                                                                                                                                                                                                                                            | DTOs e contrato HTTP não vazam relações sensíveis e delimitam as operações da conta.                                                                 |
