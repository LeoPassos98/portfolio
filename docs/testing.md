...
| Último resultado consolidado | **293 testes aprovados** na alteração de perfil da conta de acesso no backend                             |
...
Salvo a exceção indicada no smoke e2e, os arquivos `*.spec.ts` deste catálogo foram aprovados como parte da suíte de **293 testes** executada na alteração de perfil da conta de acesso no backend. Os resultados são cumulativos: não representam a quantidade criada por arquivo ou família.
...
Exercita criação, edição cadastral, situações do cadastro e da conta, perfil da conta, criação explícita de conta e consultas administrativas de Funcionários pelas rotas reais.
...
| Conta e concorrência  | Inativação e mudança real de perfil revogam todas as sessões e o contexto CSRF do alvo e mantêm sessões de terceiros. Novo login usa o perfil persistido; autoalteração conclui a resposta e revoga a própria sessão. Requisições simultâneas de suspensão ou despromoção nunca removem todos os Administradores ativos; corrida entre reativação da conta e inativação do Funcionário nunca termina com cadastro inativo e conta ativa. | As transações serializáveis e suas retentativas protegem os invariantes de acesso e continuidade administrativa. |
...
| Privacidade e OpenAPI | Não expõe hash, senha, confirmação, troca obrigatória, sessão, CSRF, OS ou histórico; documenta criação, criação de conta, edição cadastral, situações, perfil, conflitos e `conta` anulável.                                                                                                                                                                                                                                            | DTOs e contrato HTTP não vazam relações sensíveis e delimitam as operações da conta.                             |
