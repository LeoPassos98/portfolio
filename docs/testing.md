...
| Último resultado consolidado | **264 testes aprovados** em N5.4E — correção do invariante entre Funcionário e conta de acesso            |
...
Salvo a exceção indicada no smoke e2e, os arquivos `*.spec.ts` deste catálogo foram aprovados como parte da suíte de **264 testes** executada em N5.4E — correção do invariante entre Funcionário e conta de acesso. Os resultados são cumulativos: não representam a quantidade criada por arquivo ou família.
...
Exercita criação, edição cadastral, situação, criação explícita de conta e consultas administrativas de Funcionários pelas rotas reais.
...
| Conta e concorrência  | Inativação desativa a conta associada sem alterar credenciais, revoga todas as suas sessões, preserva sessões de terceiros e impede, inclusive sob requisições simultâneas, que não reste Administrador ativo. A criação simultânea da conta e a inativação nunca deixam Funcionário inativo com conta ativa.                                                                                                    | As transações serializáveis e suas retentativas protegem os invariantes de acesso e continuidade administrativa. |
...
| Segurança             | Criação, edição, situação e criação de conta exigem sessão, primeiro acesso concluído, perfil Administrador e CSRF; leituras exigem os três primeiros e não exigem CSRF.                                                                                                                                                                                                                                         | Cada operação recebe a proteção proporcional ao seu risco.                                                       |
...
| Privacidade e OpenAPI | Não expõe hash, senha, confirmação, sessão, OS ou histórico; documenta criação, criação de conta, edição cadastral, situação, conflitos e `conta` anulável.                                                                                                                                                                                                                                                      | DTOs e contrato HTTP não vazam relações sensíveis nem sugerem edição da conta.                                   |
